...
export const createRandomSalesPerformanceMetricsData = () => {
  return {
    sales: {
    ...
    avgBrandPerOrderCount: {
      current: faker.datatype.number({ min: 10, max: 10, }),
      target: faker.datatype.number({ min: 10, max: 10, }),
      businessUnitAvg: faker.datatype.number({ min: 10, max: 10, })
    },
    crossBusinessUnitOrderCount: {
