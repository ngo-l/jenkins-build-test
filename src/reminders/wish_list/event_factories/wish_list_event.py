...
from src.reminders.wish_list.settings import wish_list_reminder_settings

logger = logging.getLogger(__name__)


class WishListEventFactory(EventFactory):
    def create(self,
               data: List[dict],
               event_id: int,
               product_list_length: int,
               email_subject: str = wish_list_reminder_settings.email_subject
               ) -> ExternalEvent[EdmData]:
        # FIXME: tangled codes
        # FIXME: create a model for each row
        def construct_contact(row: dict):
            try:
                vip_num = row.get('vip_no')
                # FIXME: blast date is only for destructure
                _blast_date, email_context = (
                    lambda blast_date, **others: (blast_date, others))(**row)
